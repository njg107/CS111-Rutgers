...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        int number_of_days = numberOfDaysInMonth[month-2];
        int[] MonthReturn = new int[number_of_days];

        double drywet_prob = 0.0;
        double wetwet_prob = 0.0;

        for (int i=0;i<drywet.length;i++){
            if ((longitute==drywet[i][0]) & (latitude==drywet[i][1])){
                drywet_prob = drywet[i][month];
            }
            if ((longitute==wetwet[i][0]) & (latitude==wetwet[i][1])){
                wetwet_prob = wetwet[i][month];
            }
        }
        ...
        for (int count=0;count<number_of_days;count++){
            ...
            if (count==0){
                if (random<=0.49){MonthReturn[count]= WET;}
                if (random>=0.50){MonthReturn[count]= DRY;}      
            }
            else{
                if (MonthReturn[count-1]==1){
                    if (random<= wetwet_prob){
                        MonthReturn[count]=WET;
                    }
                    else{
                        MonthReturn[count]=DRY;
                    }
                }
                if (MonthReturn[count-1]==2){
                    if (random<= drywet_prob){
                        MonthReturn[count]=WET;
                    }
                    else{
                        MonthReturn[count]=DRY;
...
//@author: Nitin Gowda njg107@scarletmail.rutgers.edu njg107